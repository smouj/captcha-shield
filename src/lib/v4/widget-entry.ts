// @ts-nocheck
/**
 * CAPTCHA Shield v4.0 "Fortress" — Widget Entry Point
 *
 * This is the entry point for the Rollup-bundled production widget.
 * It wraps the CaptchaWidgetV4 into a self-contained IIFE that can
 * be embedded on any website via a <script> tag.
 *
 * ## Build Target
 *
 * This file is NOT used directly by the Next.js application.
 * It is consumed by `rollup.config.mjs` to produce the
 * `public/v4/widget.js` production bundle.
 *
 * ## Architecture
 *
 * ```
 * widget-entry.ts (this file)
 *   ├── CaptchaWidgetV4       → Main widget orchestrator
 *   ├── BehavioralAnalyzerV4  → 28-signal behavioral analysis
 *   ├── BotDetectorML         → ML bot detection (heuristic fallback)
 *   ├── ChallengeWasmModule   → WASM-accelerated challenge computation
 *   ├── TokenManager          → JWT token generation/verification
 *   └── i18n                  → 8-language internationalization
 * ```
 *
 * ## Security Notes
 *
 * - All code is bundled into a single IIFE with no external dependencies
 * - The output is obfuscated via javascript-obfuscator
 * - Self-defending code prevents re-formatting and debugging
 * - Shadow DOM isolation prevents style leakage
 */

/* eslint-disable no-var, prefer-rest-params, prefer-spread */

// ─── Core Widget Implementation ───────────────────────────────────────────────
//
// This entry point provides a lightweight standalone widget that can
// operate without the full React/Next.js stack. It creates an iframe
// pointing to the /widget-embed route which loads the full React widget.
//
// For the standalone build, we inline a simplified version of the widget
// that handles the iframe creation and communication protocol.
...
(function () {
  ...
  function resolveContainer(selector) {
    if (selector) {
      try { var el = document.querySelector(selector); if (el) return el; } catch (_e) { /* skip */ }
    }
    return document.getElementById('captcha-shield') || document.querySelector('[data-captcha-shield]') || null;
  }

  function encodeQuery(params: Record<string, string | number | boolean | null | undefined>) {
    var parts: string[] = [];
  ...
  function detectLanguage() {
    try { return navigator.language || (navigator as unknown as Record<string, string>).userLanguage || 'en'; } catch (e) { return 'en'; }
  }
  ...
  var Logger = {
    ...
    _styles: ['color:#10b981;font-weight:bold;', 'color:inherit;'] as string[],
    info: function (...args: unknown[]) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (console.log as any)(this._prefix, ...this._styles, ...args);
    },
    warn: function (...args: unknown[]) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (console.warn as any)(this._prefix, ...this._styles, ...args);
    },
    error: function (...args: unknown[]) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (console.error as any)(this._prefix, ...this._styles, ...args);
    },
  };
