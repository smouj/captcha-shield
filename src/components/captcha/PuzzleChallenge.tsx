...
function drawSceneToCanvas(ctx: CanvasRenderingContext2D, w: number, h: number, seed: number) {
...
function createPiecePath(ctx: CanvasRenderingContext2D, px: number, piecePx: number, h: number, shape: string) {
  if (shape === 'wave') {
    ctx.moveTo(px, 0);
    ctx.lineTo(px + piecePx, 0);
    ctx.quadraticCurveTo(px + piecePx + 8, h * 0.5, px + piecePx, h);
    ctx.lineTo(px, h);
    ctx.quadraticCurveTo(px - 8, h * 0.5, px, 0);
  } else if (shape === 'tab') {
    ctx.moveTo(px, 0);
    ctx.lineTo(px + piecePx, 0);
    ctx.lineTo(px + piecePx, h * 0.4);
    ctx.arc(px + piecePx, h * 0.45, piecePx * 0.25, -Math.PI / 2, Math.PI / 2, false);
    ctx.lineTo(px + piecePx, h);
    ctx.lineTo(px, h);
    ctx.lineTo(px, h * 0.45);
    ctx.arc(px, h * 0.4, piecePx * 0.25, Math.PI / 2, -Math.PI / 2, false);
    ctx.closePath();
  } else {
    ctx.rect(px, 0, piecePx, h);
  }
}
...
export default function PuzzleChallenge({ challengeData, onVerify }: Props) {
  ...
  // Create offscreen canvas with full scene
  useEffect(() => {
    ...
    if (!ctx) return;
    drawSceneToCanvas(ctx, canvasWidth, canvasHeight, seed);
    sceneCanvasRef.current = offscreen;
  }, [seed, canvasWidth, canvasHeight]);

  // Draw main canvas: scene + cutouts + sliding pieces
  useEffect(() => {
    const canvas = canvasRef.current;
    const sceneCanvas = sceneCanvasRef.current;
    if (!canvas || !sceneCanvas) return;
    const ctx = canvas.getContext('2d');
    if (!ctx) return;

    const w = canvasWidth, h = canvasHeight;

    // Draw full scene from offscreen canvas
    ctx.clearRect(0, 0, w, h);
    ctx.drawImage(sceneCanvas, 0, 0);

    // Draw dark cutout areas at target positions
    for (const piece of pieces) {
      ...
      ctx.beginPath();
      createPiecePath(ctx, targetPx, piecePx, h, piece.shape);
      ctx.fill();
    ...
    // Draw sliding pieces clipped from offscreen scene
    for (let i = 0; i < pieces.length; i++) {
      ...
      // Clip to piece shape at current position
      ctx.beginPath();
      createPiecePath(ctx, currentPx, piecePx, h, piece.shape);
      ctx.clip();

      // Draw the full scene into the clipped area (shifted so the piece shows its scene content)
      ctx.drawImage(sceneCanvas, 0, 0);

      // Slight highlight overlay so the piece is distinguishable
  ...
  }, [sliderPositions, pieces, seed, activePiece, canvasWidth, canvasHeight]);
